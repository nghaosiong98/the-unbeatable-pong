package pacman.controllers.examples.evaluators;

import pacman.controllers.examples.MonteCarloGameNode;
import pacman.controllers.examples.MonteCarloTree;
	...
	public void evaluateTree(MonteCarloTree tree) {
		...
		for (MonteCarloGameNode child: tree.getPacManChildren()) {
			...
			while (!game.isJunction(game.getPacmanCurrentNodeIndex())) {
				game.advanceGame(MyMonteCarloPacMan.nonJunctionSim(game), ghosts.getMove(game, 40));
			}
