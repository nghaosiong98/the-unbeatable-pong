...
def custom_heuristic_junshou(game, player):
    if game.is_loser(player):
        return float("-inf")
    if game.is_winner(player):
        return float("inf")
...
# Customize heuristic func
def custom_heuristic_robinson(game, player):
    if game.is_loser(player):
        return float("-inf")
    if game.is_winner(player):
        return float("inf")
...
def my_custom_heuristic_haosiong(game, player):
    if game.is_loser(player):
        return float("-inf")
    ...
    non_blank_spaces = 49 - len(game.get_blank_spaces())

    center_spaces = [(3, 3)]

    if non_blank_spaces == 3:
        if game.get_player_location(player) in center_spaces:
            return float("inf")

    return float(my_moves - opponent_moves - non_blank_spaces * 0.01)


def my_custom_heuristic_marina(game, player):
    if game.is_loser(player):
        return float("-inf")
