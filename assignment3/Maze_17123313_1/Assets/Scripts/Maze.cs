...
public class Maze : MonoBehaviour
{
    ...
    public IEnumerator GenerateGTA()
    {
        // Initialize the first cell as entry cell
        Cell currentCell = PlaceRandomEntryCell();
        currentLength++;
    ...
    public IEnumerator GenerateBinary() {
        for (int x = 0; x < width; x ++) {
            for (int z = 0; z < height; z ++) {
                CellLocation current_location = new CellLocation(x, z);
                Cell current_cell = PlaceCell(current_location);
                if (x == 0 && z == 0) {
                    //current_cell.Material = entryCellMaterial;
                    entryCell = current_cell;
                    entryCell.Material = entryCellMaterial;

                }  
                if (x == height-1 && z == width-1) {
                    exitCell = current_cell;
                    CellDirection exitCellDirection = GetDirectionThatLeadstoOutOfBound(exitCell);
                    exitCell.Material = exitCellMaterial;
                    exitCell.AddConnection(exitCellDirection);

                }
                List<int> path = new List<int>();//if dead end
                if (x > 0) path.Add(2);
                if (z > 0) path.Add(1);
                
                if (path.Count > 0) {
                    path.Shuffle();
                    CellDirection target_path = (CellDirection)path[0];
                    CellLocation neighbor = current_cell.Location + target_path.ToRelativeCellLocation();
                    CellDirection last_direction = target_path.GetOpposite();
                    current_cell.AddConnection(target_path);
                    cells[neighbor.x, neighbor.z].AddConnection(last_direction);
                }
                currentLength++;
                activeCells.Add(current_cell);
                cellDistances[current_cell.Location.x, current_cell.Location.z] = currentLength;
                yield return new WaitForSeconds(delay);
            }
        }
